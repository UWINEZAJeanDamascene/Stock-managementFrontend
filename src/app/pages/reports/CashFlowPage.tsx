...
import { reportsApi, CashFlowReport, CFSection, CFLineItem } from '@/lib/api';
...
function CashFlowSection({
  ...
  const totalLabel = title === 'Operating' ? 'net_cash_from_operating'
    : title === 'Investing' ? 'net_cash_from_investing'
  ...
  return (
    <div className="mt-3">
      ...
      {expanded && (
        <div className="border-l-2 ml-2">
          ...
          {section.inflows.length > 0 && (
            <>
              ...
              {section.inflows.map((item, idx) => {
                const compItem = comparative?.inflows.find(c => c.source_type === item.source_type);
                return (
                  <SectionRow
                    key={`in-${idx}`}
                    label={item.label}
          ...
          {section.outflows.length > 0 && (
            <>
              ...
              {section.outflows.map((item, idx) => {
                const compItem = comparative?.outflows.find(c => c.source_type === item.source_type);
                return (
                  <SectionRow
                    key={`out-${idx}`}
                    label={item.label}
          ...
          {section.inflows.length === 0 && section.outflows.length === 0 && (
            <div className="text-xs text-muted-foreground pl-4 py-2">No cash movements in this section</div>
          )}
        </div>
      )}
      ...
      <SectionRow
        ...
        current={net}
        comparative={comparative ? (comparative[totalLabel as keyof CFSection] as number) : undefined}
        bold
        isNegative={net < 0}
      />
    </div>
      ...
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Statement of Cash Flows — IAS 7 (Direct Method)</p>
          </div>
            ...
            <CardContent className="pt-4 pb-6">
              ...
              <div className="mt-4 p-4 rounded-lg border-2 bg-muted/30">
                <div className="flex items-center">
                  ...
                  <span className={`w-36 text-right font-mono text-lg font-bold ${cur.net_change_in_cash >= 0 ? 'text-foreground' : 'text-destructive'}`}>
                    {fmt(cur.net_change_in_cash)}
                  </span>
                  {comp && (
                    <span className={`w-36 text-right font-mono text-lg font-bold ${comp.net_change_in_cash >= 0 ? 'text-foreground' : 'text-destructive'}`}>
              ...
              <div className="mt-4 p-4 rounded-lg border-2 bg-primary/5">
                <div className="flex items-center">
                  ...
                  <span className={`w-36 text-right font-mono text-lg font-bold ${cur.closing_cash_balance >= 0 ? 'text-foreground' : 'text-destructive'}`}>
                    {fmt(cur.closing_cash_balance)}
                  </span>
                  {comp && (
                    <span className={`w-36 text-right font-mono text-lg font-bold ${comp.closing_cash_balance >= 0 ? 'text-foreground' : 'text-destructive'}`}>
              ...
              {cur.is_reconciled ? (
                ...
                <div className="mt-3 flex items-center justify-center gap-2 text-sm">
                  <Badge variant="secondary" className="bg-red-100/80 text-red-800 dark:bg-red-900/30 dark:text-red-400">
                    ...
                    Not reconciled — difference of {fmt(cur.reconciliation_diff)}
                  </Badge>
                </div>
              )}

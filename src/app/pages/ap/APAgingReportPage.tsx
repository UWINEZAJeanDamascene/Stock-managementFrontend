...
export default function APAgingReportPage() {
  ...
  const [verification, setVerification] = useState<{ verified: boolean; discrepancyCount: number } | null>(null);
  ...
  const fetchAgingReport = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        const agingResponse = response.data?.aging;
        const agingDataArray = Array.isArray(agingResponse?.data) ? agingResponse.data : [];
      ...
      <div className="container mx-auto py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">{t('apAging.title', 'AP Aging Report')}</h1>
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={fetchAgingReport} disabled={loading}>
        ...
        {/* Verification Status */}
        {verification && (
          <Card className={verification.verified ? 'border-green-200 bg-green-50' : 'border-yellow-200 bg-yellow-50'}>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {verification.verified
                    ...
                    : t('apAging.discrepancyCount', '{{count}} discrepancies require attention', { count: verification.discrepancyCount })}
                </p>
              </div>
              {!verification.verified && (
                <Button 
          ...
          <CardContent className="py-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <Button onClick={fetchAgingReport} disabled={loading} className="w-full">
                  {loading ? (
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mt-2">
                  {t('apAging.selectFilters', 'Select filters and generate the report')}
                </p>
              </div>
            ) : (
                  ...
                  <TableBody>
                    {agingData.map((row) => (
                      <TableRow key={row.supplier_id}>
                        ...
                        <TableCell className="text-right">{getAgingBadge(parseFloat(row.not_yet_due))}</TableCell>
                        <TableCell className="text-right">{getAgingBadge(parseFloat(row.days_1_30))}</TableCell>
                        <TableCell className="text-right">{getAgingBadge(parseFloat(row.days_31_60))}</TableCell>
                        <TableCell className="text-right">{getAgingBadge(parseFloat(row.days_61_90))}</TableCell>
                        <TableCell className="text-right">{getAgingBadge(parseFloat(row.days_90_plus))}</TableCell>
                        ...
                        <TableCell>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => navigate(`/ap-reconciliation/suppliers/${row.supplier_id}/statement`)}
                          >
